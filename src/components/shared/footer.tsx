...
export function Footer() {
  ...
  return (
    <footer ref={footerRef} className="relative w-full overflow-hidden bg-black">
      <motion.div className="absolute inset-0 z-0" style={{ y: robotY }}>
      ...
      <motion.div className="relative z-10 w-full" style={{ y: textY }}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-10 gap-12 sm:gap-14 lg:gap-20 xl:gap-28">
            <div className="lg:col-span-4">
              ...
              <div className="space-y-4 sm:space-y-6 mb-8 sm:mb-10">
                <p className="text-sm text-gray-400 leading-relaxed tracking-wide">
                  © {new Date().getFullYear()} Flo Mobility<br />Manufactured in India 🇮🇳
                </p>
                <p className="text-sm text-gray-500 leading-relaxed">
                  HSR Layout, Bengaluru<br />Karnataka, India
                </p>
              </div>
            ...
            <div className="lg:col-span-6">
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-8 sm:gap-x-10 lg:gap-x-16 xl:gap-x-20 gap-y-10 sm:gap-y-12">
                <div>
                  <h3 className="text-[10px] sm:text-xs font-bold text-gray-600 uppercase tracking-[0.15em] mb-4 sm:mb-6 lg:mb-8">Navigation</h3>
                  <nav className="flex flex-col space-y-3 sm:space-y-4">
                    {FOOTER_LINKS.col1.map((link) => (
                      <Link key={link.name} href={link.href} className="text-[15px] sm:text-[17px] lg:text-[19px] font-normal text-white/90 hover:text-[#7ccd54] transition-all duration-200 hover:translate-x-1" style={{ fontFamily: "var(--font-inter)" }}>
                        {link.name}
                      </Link>
                    ))}
                  </nav>
                ...
                <div>
                  <h3 className="text-[10px] sm:text-xs font-bold text-gray-600 uppercase tracking-[0.15em] mb-4 sm:mb-6 lg:mb-8">Offerings</h3>
                  <nav className="flex flex-col space-y-3 sm:space-y-4">
                ...
                <div>
                  <h3 className="text-[10px] sm:text-xs font-bold text-gray-600 uppercase tracking-[0.15em] mb-4 sm:mb-6 lg:mb-8">Join Us</h3>
                  <nav className="flex flex-col space-y-3 sm:space-y-4">
          ...
          <Link href="/offerings/material-movement" className="group block">
            <div className="flex items-center justify-between">
              <div className="flex-1">
                <p className="text-[10px] sm:text-[11px] md:text-xs text-gray-600 uppercase tracking-[0.2em] mb-3 sm:mb-4 lg:mb-5 font-semibold">NEXT PAGE</p>
                <h3 className="text-[clamp(2.5rem,12vw,7rem)] font-bold text-[#7ccd54] group-hover:text-[#a9fe7e] transition-all duration-300 leading-none tracking-tight" style={{ fontFamily: "var(--font-dm-sans)", textShadow: "0 0 60px rgba(124,205,84,0.2)" }}>MMR</h3>
              </div>
